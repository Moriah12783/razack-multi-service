...
function vLine(v) {
  const st = (v.statut || v.status) === 'sold' ? 'VENDU' : 'disponible';
  const km = v.km != null ? `${v.km} km, ` : '';
  return `- ${v.brand} ${v.model} ${v.year} (${v.type}, ${km}${st}) — ${fmtPrice(v.price)} — fiche: ${SITE}${v.url}`;
}
function locLine(v) {
  const st = (v.statut || v.status) === 'sold' ? 'indisponible' : 'disponible';
  return `- ${v.brand} ${v.model} ${v.year} (${v.type}, ${v.formule}, ${st}) — dès ${fmtPrice(v.priceDay)}/jour — fiche: ${SITE}${v.url}`;
}
function immoLine(b) {
  const st = (b.statut || b.status) === 'sold' ? 'indisponible' : 'disponible';
  return `- ${b.titre} (${b.type}, ${b.quartier}, ${st}) — ${fmtPrice(b.prix)}${b.unite ? '/' + b.unite : ''} — fiche: ${SITE}${b.url}`;
}
function meubleLine(m) {
  const st = (m.statut || m.status) === 'sold' ? 'VENDU' : 'disponible';
  return `- ${m.titre} (${m.cat}, ${st}) — ${fmtPrice(m.prix)} — fiche: ${SITE}${m.url}`;
}
...
function buildKb(data) {
  ...
  parts.push('\n## GUIDES (blog)\n' + (data.articles || []).map(a => `- ${a.title} — ${SITE}${a.url}`).join('\n'));
  return parts.join('\n');
}

module.exports = { buildKb, SITE };
