...
const { formatFrDate } = require('../blog');
...
<section class="section"><div class="container" style="max-width:820px">
  ...
  ${header}
  ${a.date ? `<p style="color:var(--gray-400);font-size:.85rem;margin:12px 0 24px">Publié le ${esc(formatFrDate(a.date))}</p>` : ''}
  ...
  return renderPage({ title: `${a.title} | RAZAK Multi Service`, description: a.description, canonical, image: img, ogType: 'article', headExtra, main });
}
